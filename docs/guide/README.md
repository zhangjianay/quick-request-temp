...
[**Quick Request**](https://plugins.jetbrains.com/plugin/16988-fast-request)  是一个强大的restful
api工具包插件，可以根据已有的方法帮助您快速生成url和params。`Quick Request = API调试工具+API管理工具`。 它有一个漂亮的界面来完成请求、检查服务器响应、存储你的api请求和导出api请求。
插件帮助你在IDEA界面内更快更高效得调试你的API

对比与HTTP Client，Quick Request不仅拥有HTTP Client内置的功能，还提供了友好易懂直观的界面，让使用者调试API的时候能够更加方便、简捷，同时各种类型参数也提供了不同的定制方式，更加灵活。
...
欢迎各路好汉一起来参与完善 Quick Request，我们期待你的 PR！

* 贡献代码：代码地址 [Quick Request](https://github.com/dromara/fast-request)，欢迎提交 Issue 或者 Pull Requests
